...
module Torb
  class Web < Sinatra::Base
    ...
    get '/api/users/:id', login_required: true do |user_id|
      ...
      rows_reserve = db.xquery('SELECT r.*, s.rank AS sheet_rank, s.num AS sheet_num FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id WHERE r.user_id = ? ORDER BY updated_at DESC LIMIT 5', user['id'])
      ...
      rows_events = db.xquery('SELECT event_id FROM reservations WHERE user_id = ? GROUP BY event_id ORDER BY MAX(updated_at) DESC LIMIT 5', user['id'])
