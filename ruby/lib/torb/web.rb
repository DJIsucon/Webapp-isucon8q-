...
module Torb
  class Web < Sinatra::Base
    ...
    get '/api/users/:id', login_required: true do |user_id|
      ...
      rows = db.xquery('SELECT r.*, s.rank AS sheet_rank, s.num AS sheet_num FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id WHERE r.user_id = ? ORDER BY updated_at DESC LIMIT 5', user['id'])
    ...
    post '/api/events/:id/actions/reserve', login_required: true do |event_id|
      ...
      reservation_id = nil
      loop do
        sheet = db.xquery('SELECT * FROM sheets WHERE id IN (SELECT sheet_id FROM active_reservations WHERE event_id = ? FOR UPDATE) AND `rank` = ? ORDER BY RAND()', event['id'], rank).first
